...
";
    ...
    public function insertProfile($fname, $lname, $gender, $location, $makeModelYear, $mileage) {

        include './classes/dbconnect.php';

        $dbs = $db->prepare('insert into profiles set fname = :fname, lname = :lname, gender = :gender, location = :location, mileage = :milege');

        // you must bind the data before you execute
        $dbs->bindParam(':fname', $fname, PDO::PARAM_STR);
        $dbs->bindParam(':lname', $lname, PDO::PARAM_STR);
        $dbs->bindParam(':gender', $gender, PDO::PARAM_STR);
        $dbs->bindParam(':location', $location, PDO::PARAM_STR);
        $dbs->bindParam(':mileage', $mileage, PDO::PARAM_STR);
    ...
    include './classes/dbconnect.php';
    $dbs = $db->prepare("select * from user_profile WHERE ". $_SESSION['userid'] ." = profile_id"); 
    if ( $dbs->execute() && $dbs->rowCount() > 0 ) {          
